...
def process_one(query, sub_queries, write_file, i):
    ...
    for sub_query in sub_queries:
        ...
        for key in join_cond:
            join_cond_vals.append(key)

        joins = list(itertools.permutations(query_tables, 2))
        for join_ in joins:
            # print(join_[0]+' '+join_[1])
            if join_[0]+' '+join_[1] in join_cond_vals:
                query_str += join_cond[join_[0]+' '+join_[1]]+ " AND "
...
def generate_all_single_table_queries(raw_file="join_est_record_job.txt", raw_sql="../raw_sql.sql", write_file_path="sub_queries.sql"):
    f=open(POSTGRES_DATA_DIR+raw_file)
    lines=f.readlines()
    f.close()
    f=open(raw_sql)
    # f = open("/Users/hanyuxing/Desktop/CardEstBenchmark/stats/stats_CEB.sql")
    ...
    file = open(POSTGRES_DATA_DIR+ "join_est_record_job_process.txt","r+")
    write_file = open(POSTGRES_DATA_DIR+write_file_path, "w+")
    string = file.read()
    i = 0
    per_queries = string.split("SELECT")[1:]
    #print(len(per_queries))
